       ...
       private var centre: CGPoint = .zero
        ...
        init(withCentre centre: CGPoint, radius: CGFloat, itemSize: CGSize, angularSpacing: CGFloat) {
         ...
         required init?(coder: NSCoder) {
            ...
            self.centre = CGPoint(x: UIScreen.main.bounds.width / 2, y: UIScreen.main.bounds.height / 2)
            self.radius = 150 // Adjust as needed
            self.itemSize = CGSize(width: 80, height: 80) // Adjust as needed
            self.angularSpacing = 20
            self._startAngle = .pi
            self._endAngle = 0
        }
        ...
        override var collectionViewContentSize: CGSize {
            ...
            let scrollableContentWidth = CGFloat(remainingItemsCount + 1) * angleOfEachItem * radius / (2.0 * CGFloat.pi / visibleAngle)
